...
> Status: alpha.21/22 rejected; alpha.23 active candidate
> Intent epoch: `pilot-native-sidebar-v2`
> Candidate branch: `codex/pilot-native-sidebar-alpha23`
...
| R1 | Contain header/body/navigation inside the dock | descendant bounds stay inside panel at every tab | passed at `b5f56d5` |
| R2 | Add Pilot-owned right-sidebar resizing | mouse + keyboard drag, 240–560 px, concession tests | passed at Pilot `059c09cd0` |
| R3 | Bind panel state to the current strict session | blank/current/session-switch matrix | passed at `630d3e5` |
| R4 | Admit alpha.23 in real Electron desktop | 1280×720, 1600×900, 1920×1080, 200% zoom | passed |
...
- Files in this branch and any alpha.23 package are candidates. Alpha.19,
  alpha.20, alpha.21, and alpha.22 are rejected runtime candidates. Alpha.21 passed a
...
- Alpha.22 fixed the visible layout but reused a one-shot project-list request;
  repeated Electron starts could silently fall back to an empty Work page when
  that first RPC raced Host readiness.
- Green unit tests do not upgrade alpha.23 to a partner release.
...
R1–R4 pass. Resume at P4 with a real configured model and artifact-download
journey. The Pilot runtime companion is preserved as a mail-formatted patch
under `integrations/pilot-harness/`; its validation branch remains local and
must not be pushed to the third-party remote without maintainer permission.
